...
function VenueUpdate() {
    ...
    const handleRemoveCourt = async (index) => {
        if (!window.confirm("Are you sure you want to delete this court?")) return;
        const courtToDelete = courts[index];

        try {
            // Only call delete API if court has an ID (i.e., it's not a newly added unsaved court)
            if (courtToDelete.id) {
                await apiRequest.delete("/court/deletecourt", {
                    data: { courtId: courtToDelete.id },
                });
            }
            ...
            setCurrentVenue(updatedVenue); // Update state too if necessary
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error("Error updating venue:", err);
            setError(
                `Error updating venue: ${err.response?.data?.message || err.response?.data || err.message || "Unknown error"
                }`
            );
                            ...
                            <div className="manager-courts-container">
                                ...
                                {courts.map((court, index) => (
                                    <div key={index} className="manager-court-item">
                                        ...
                                        <button className="remove-btn" type="button" onClick={() => handleRemoveCourt(index)}>
                                            ❌ Remove
                                        </button>
                                    </div>
                                ))}
